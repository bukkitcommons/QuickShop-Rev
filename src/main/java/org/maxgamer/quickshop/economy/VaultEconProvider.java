...
public class VaultEconProvider implements EconomyProvider, Listener {
    ...
    private boolean setupEconomy() {
        ...
        if (this.vault.getName() == null || this.vault.getName().isEmpty()) {
            ShopLogger.instance().warning("Current economy plugin not correct process all request, this usually cause by irregular code, you should report this issue to your economy plugin author or use other economy QuickShop.instance().");
            ShopLogger.instance().warning("This is technical information, please send this to economy plugin author: " + "VaultEconomyProvider.getName() return a null or empty.");
        } else {
        ...
        Util.trace("Economy service listener was registered.");
        return true;
    }
    ...
    public boolean deposit(@NotNull UUID name, double amount) {
        ...
        try {
            ...
            t.printStackTrace();
            ShopLogger.instance().warning("This seems not QuickShop fault, you should cotact with your economy plugin author. (" + getProviderName() + ")");
            return false;
        }
    }
    ...
    public double getBalance(@NotNull UUID name) {
        ...
        try {
            ...
            QuickShop.instance().getSentryErrorReporter().ignoreThrow();
            t.printStackTrace();
            ShopLogger.instance().warning("This seems not QuickShop fault, you should contact with your economy plugin author. (" + getProviderName() + ")");
            return 0.0;
        }
    }
}
