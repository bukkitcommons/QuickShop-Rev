    ...
    new BukkitRunnable() {
      ...
      public void run() {
        ...
        for (Shop shop : plugin.getShopLoader().getShopsInDatabase()) {
          ...
          /*
           * shop.getItem() is a constant that has NotNull annotations so. if (shop.getItem() ==
           * null) { sender.sendMessage(ChatColor.YELLOW + "Shop " + shop +
           * " removing cause item data is damaged."); shop.delete(); continue; }
           */
          ...
          /*
           * shop.getLocation() is a constant that has NotNull annotations so. if
           * (shop.getLocation() == null) { sender.sendMessage(ChatColor.YELLOW + "Shop " + shop +
           * " removing cause location data is damaged."); shop.delete(); continue; }
           */
