/*
 * This file is a part of project QuickShop, the name is InternalListener.java Copyright (C)
...
public class InternalListener implements Listener {
  ...
  public void shopCreate(ShopCreateEvent event) {
    plugin.log("Player " + event.getPlayer().getName() + " created a shop at location "
  ...
  public void shopModeratorChanges(ShopModeratorChangedEvent event) {
    plugin.log("Shop at location " + event.getShop().getLocation() + " moderator was changed to "
        + event.getModerator());
  }

  @EventHandler(priority = EventPriority.MONITOR, ignoreCancelled = true)
  public void shopPriceChanges(ShopPriceChangeEvent event) {
    plugin.log("Shop at location " + event.getShop().getLocation() + " price was changed from "
  ...
  public void shopPurchase(ShopSuccessPurchaseEvent event) {
    if (event.getShop().getShopType() == ShopType.BUYING) {
      plugin.log("Player " + event.getPlayer().getName() + " sold " + event.getShop().ownerName()
          + " shop " + event.getShop() + " for items x" + event.getAmount() + " for "
          + plugin.getEconomy().format(event.getBalance()) + " ("
          + plugin.getEconomy().format(event.getTax()) + " tax).");
    }
    if (event.getShop().getShopType() == ShopType.SELLING) {
      plugin.log("Player " + event.getPlayer().getName() + " bought " + event.getShop().ownerName()
          + " shop " + event.getShop() + " for items x" + event.getAmount() + " for "
          + plugin.getEconomy().format(event.getBalance()) + " ("
          + plugin.getEconomy().format(event.getTax()) + " tax).");
    }
