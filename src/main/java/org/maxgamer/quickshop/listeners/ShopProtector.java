...
public class ShopProtector implements Listener {
    private static void handleProtection(@NotNull Location location, @NotNull Runnable protection) {
        handleProtection(location, true, protection, shop -> {});
    }
    
    private static void handleProtection(@NotNull Location location, @NotNull boolean predicate, @NotNull Runnable protection, @NotNull Consumer<ChestShop> orElse) {
        
        Shop.getManager().getLoadedShopFrom(location)
                
                .ifPresent(shop -> {
                    if (predicate)
                        protection.run();
                    else
                        orElse.accept((ChestShop) shop);
                });
    }
    ...
    public void onBlockExplode(BlockExplodeEvent event) {
        for (Block block : event.blockList())
            handleProtection(block.getLocation(), BaseConfig.explosionProtection, () -> event.setCancelled(true), shop -> Shop.getLoader().delete(shop));
    }
