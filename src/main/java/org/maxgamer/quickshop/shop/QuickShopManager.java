...
public class QuickShopManager implements ShopManager {
    ...
    public BasicShop load(@NotNull World world, @NotNull ShopData data) throws JsonSyntaxException, InvalidConfigurationException {
        Map<Long, Map<Long, BasicShop>> inWorld = loadedShops.computeIfAbsent(data.world(), s -> LongHashMap.withExpectedSize(8));
        
        Map<Long, BasicShop> inChunk = inWorld.computeIfAbsent(Utils.chunkKey(data.x() >> 4, data.z() >> 4), s -> LongHashMap.withExpectedSize(16));
        
        ContainerQuickShop shop;
        
        if (data.type() == ShopType.SELLING)
            shop = new QuickShopSeller(ShopLocation.from(world, data.x(), data.y(), data.z()), (double) data.price().stack(), StackItem.of(ItemUtils.deserialize(data.item())), data.moderators(), data.unlimited(), data.type());
        else
            shop = new QuickShopBuyer(ShopLocation.from(world, data.x(), data.y(), data.z()), (double) data.price().stack(), StackItem.of(ItemUtils.deserialize(data.item())), data.moderators(), data.unlimited(), data.type());
    ...
    public BasicShop load(@NotNull BasicShop shop) {
        Map<Long, Map<Long, BasicShop>> inWorld = loadedShops.computeIfAbsent(shop.location().worldName(), s -> LongHashMap.withExpectedSize(8));
        
        Map<Long, BasicShop> inChunk = inWorld.computeIfAbsent(Utils.chunkKey(shop.location().x() >> 4, shop.location().z() >> 4), s -> LongHashMap.withExpectedSize(16));
        
        inChunk.put(Utils.blockKey(shop.location().x(), shop.location().y(), shop.location().z()), shop);
        shop.load();
    ...
    public void viewLoadedShops(Consumer<Collection<BasicShop>> con) {
        loadedShops.values().forEach(inChunk -> inChunk.values().forEach(blockMap -> con.accept(blockMap.values())));
    }
    
    private static class BlockDataWrapper {
        private final static void handleWaterLogged(BlockState signState) throws NoClassDefFoundError, ClassNotFoundException {
        ...
        private final static void handleWallSignFacing(BlockFace chestFace, BlockState signState) throws NoClassDefFoundError, ClassNotFoundException {
            org.bukkit.block.data.type.WallSign signData = (org.bukkit.block.data.type.WallSign) signState.getBlockData();
    ...
    public void createShop(@NotNull BasicShop shop, @NotNull ShopCreator info) {
        ...
        if (info.sign() != null && BaseConfig.autoSign) {
            ...
            try {
                QuickShop.instance().getDatabaseHelper().createShop(shop.moderator().serialize(), Stack.of(shop.price()), shop.stack(), shop.unlimited() ? 1 : 0, shop.type().toID(), location.worldName(), location.x(), location.y(), location.z());
    ...
    public ShopViewer getLoadedShopAt(@NotNull ShopLocation loc) {
        @Nullable
        Map<Long, BasicShop> inChunk = getLoadedShopsInChunk(loc.chunk());
        if (inChunk == null)
            return ShopViewer.empty();
    ...
    public ShopViewer getLoadedShopAt(String world, int x, int y, int z) {
        @Nullable
        Map<Long, BasicShop> inChunk = getLoadedShopsInChunk(world, x >> 4, z >> 4);
        if (inChunk == null)
            return ShopViewer.empty();
    ...
    public void unload(@NotNull BasicShop shop) {
        ...
        Map<Long, Map<Long, BasicShop>> inWorld = loadedShops.get(location.worldName());
        
        if (inWorld != null) {
            Map<Long, BasicShop> inChunk = inWorld.get(Utils.chunkKey(location.x() >> 4, location.z() >> 4));
