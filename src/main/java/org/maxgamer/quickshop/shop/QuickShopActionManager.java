...
public class QuickShopActionManager implements ShopActionManager {
     ...
     * Action manager
     */
    private final Map<UUID, ShopActionData> actionData = Maps.newConcurrentMap();
    ...
    @Override
    public boolean buy(@NotNull Player p, @NotNull String message, @NotNull BuyerShop shop, int amount, @NotNull ShopSnapshot info) {
        
        // No enough shop space
        int space = shop.getRemainingSpace();
        if (!shop.unlimited() && space < amount) {
            p.sendMessage(Shop.getLocaleManager().get("shop-has-no-space", p, "" + space, ItemUtils.getItemStackName(shop.stack())));
            return false;
        }
        ...
        if (amount > count) {
            p.sendMessage(Shop.getLocaleManager().get("you-dont-have-that-many-items", p, "" + count * info.stack().<ItemStack>stack().getAmount(), ItemUtils.getItemStackName(shop.stack())));
            return false;
        }
    ...
    private boolean actionBuy0(@NotNull Player p, @NotNull String message, @NotNull BuyerShop shop, int amount, @NotNull ShopSnapshot info) {
        ...
        if (shouldPayOwner) {
            boolean withdrawOwner = QuickShop.instance().getEconomy().withdraw(shop.getOwner(), totalPrice); // Withdraw
                                                                                                             // owner's
                                                                                                             // money
            if (!withdrawOwner) {
                p.sendMessage(Shop.getLocaleManager().get("the-owner-cant-afford-to-buy-from-you", p, format(totalPrice), format(QuickShop.instance().getEconomy().getBalance(shop.getOwner()))));
                return false;
            }
        }
        ...
        boolean depositSeller = QuickShop.instance().getEconomy().deposit(p.getUniqueId(), moneyAfterTax); // Deposit
                                                                                                           // player's
                                                                                                           // money
        if (!depositSeller) {
            ShopLogger.instance().warning("Failed to deposit the money " + moneyAfterTax + " to player " + p.getName());
            /* Rollback the trade */
            if (shouldPayOwner)
                if (!QuickShop.instance().getEconomy().deposit(shop.getOwner(), totalPrice))
                    ShopLogger.instance().warning("Failed to rollback the purchase actions for player " + Bukkit.getOfflinePlayer(shop.getOwner()).getName());
        ...
        int space = shop.getRemainingSpace();
        if (space == amount) {
            String msg = "\n" + Shop.getLocaleManager().get("shop-out-of-space", p, "" + shop.location().x(), "" + shop.location().y(), "" + shop.location().z());
    ...
    public void create(@NotNull Player p, @NotNull ShopCreator info, @NotNull String message, boolean bypassProtectionChecks) {
        ...
        if (BlockUtils.getSecondHalf(info.location().block()).isPresent() && !QuickShopPermissionManager.instance().has(p, "quickshop.create.double")) {
        ...
        if (BaseConfig.autoSign && !BaseConfig.allowNoSign) {
            ...
            if (!BlockUtils.isAir(signType) && signType != Material.WATER) {
                ...
                if (!wallSign || !Arrays.stream(((Sign) info.sign().getState()).getLines()).allMatch(String::isEmpty)) {
            ...
            if (BaseConfig.integerPriceOnly)
                ...
                if (processedDouble.length > 1) {
                    ...
                    if (processedDouble[1].length() > maximumDigitsLimit && maximumDigitsLimit != -1) {
                        p.sendMessage(Shop.getLocaleManager().get("digits-reach-the-limit", p, String.valueOf(maximumDigitsLimit)));
                        return;
                    }
                }
            }
        ...
        if (BaseConfig.allowFreeShops) {
            if (price != 0 && price < minPrice) {
                p.sendMessage(Shop.getLocaleManager().get("price-too-cheap", p, (decFormat) ? ShopUtils.formatPrice(minPrice) : "" + minPrice));
                return;
            }
        } else
            if (price < minPrice) {
        ...
        if (price_limit != -1)
            if (price > price_limit) {
                p.sendMessage(Shop.getLocaleManager().get("price-too-high", p, (decFormat) ? ShopUtils.formatPrice(price_limit) : "" + price_limit));
                return;
            }
        ...
        if (priceRestriction != null)
            if (price < priceRestriction.getKey() || price > priceRestriction.getValue())
                ...
                p.sendMessage(Shop.getLocaleManager().get("restricted-prices", p, ItemUtils.getItemStackName(info.stack().stack()), String.valueOf(priceRestriction.getKey()), String.valueOf(priceRestriction.getValue())));
        ...
        if (createCost > 0) {
            if (!QuickShop.instance().getEconomy().withdraw(p.getUniqueId(), createCost)) {
                p.sendMessage(Shop.getLocaleManager().get("you-cant-afford-a-new-shop", p, format(createCost)));
                return;
            }
            try {
                String taxAccount = BaseConfig.taxAccount;
                if (taxAccount != null)
                    QuickShop.instance().getEconomy().deposit(Bukkit.getOfflinePlayer(taxAccount).getUniqueId(), createCost);
            } catch (Exception e2) {
                e2.printStackTrace();
                ShopLogger.instance().log(Level.WARNING, "QuickShop can't pay tax to account in config.yml, Please set tax account name to a existing player!");
            }
        }
        ...
        if (shop.converse().isPresent()) {
            ShopViewer nextTo = shop.getAttachedShop();
            if (nextTo.isPresent() && nextTo.get().<Double>price() > shop.price())
    ...
    @Override
    public void sell(@NotNull Player p, @NotNull String message, @NotNull SellerShop shop, int amount, @NotNull ShopSnapshot info) {
        ...
        if (stock < amount) {
            p.sendMessage(Shop.getLocaleManager().get("shop-stock-too-low", p, String.valueOf(stock), ItemUtils.getItemStackName(shop.stack()) + stacks));
            return;
        }
        if (amount < 1) {
            // & Dumber
        ...
        int pSpace = ShopUtils.countSpaces(p.getInventory(), shop.stack());
        if (amount > pSpace) {
        ...
        boolean successA = QuickShop.instance().getEconomy().withdraw(p.getUniqueId(), total); // Withdraw
                                                                                               // owner's
                                                                                               // money
        if (!successA) {
            p.sendMessage(Shop.getLocaleManager().get("you-cant-afford-to-buy", p, Objects.requireNonNull(format(total)), Objects.requireNonNull(format(QuickShop.instance().getEconomy().getBalance(p.getUniqueId())))));
            return;
        }
        boolean shouldPayOwner = !shop.unlimited() || BaseConfig.payUnlimitedShopOwners;
        if (shouldPayOwner) {
            ...
            if (!successB) {
                ShopLogger.instance().warning("Failed to deposit the money for player " + Bukkit.getOfflinePlayer(shop.getOwner()));
                /* Rollback the trade */
        ...
        if (stock == amount)
            msg += "\n" + Shop.getLocaleManager().get("shop-out-of-stock", p, "" + shop.location().x(), "" + shop.location().y(), "" + shop.location().z(), ItemUtils.getItemStackName(shop.stack()));
    ...
    @Override
    public void trade(@NotNull Player p, @NotNull ShopSnapshot info, @NotNull String message) {
    ...
    public void handleChat(@NotNull Player p, @NotNull String msg, boolean bypassProtectionChecks, boolean sync) {
        ...
        Runnable runnable = () -> {
            ...
            if (!info.location().worldName().equals(p.getLocation().getWorld().getName()) || info.location().bukkit().distanceSquared(p.getLocation()) > 25) {
