...
public abstract class DisplayDataMatcher {
    ...
    public static DisplayData create(@NotNull ItemStack item) {
        try {
            ...
            YamlConfiguration conf = QuickShop.instance().getConfigurationManager().get(DisplayConfig.class).conf();
            
            if (conf != null) {
                @Nullable
                DisplayDataMatchesResult dataArmourStand = DisplayDataMatcher.matchDataFor(conf, "settings.specifics.armor-stand", item, DisplayType.ARMOR_STAND);
                
                // Return if vaildate is not need (accurate matches)
                if (dataArmourStand != null && !dataArmourStand.needVaildate)
                    return dataArmourStand.fixer ? (def.type() == dataArmourStand.data.type() ? dataArmourStand.data : def) : dataArmourStand.data;
                
                @Nullable
                DisplayDataMatchesResult dataDroppedItem = DisplayDataMatcher.matchDataFor(conf, "settings.specifics.dropped-item", item, DisplayType.DROPPED_ITEM);
                
                // Return or consider the vaildate-need data if not exists.
                if (dataDroppedItem != null)
                    return dataDroppedItem.fixer ? (def.type() == dataDroppedItem.data.type() ? dataDroppedItem.data : def) : dataDroppedItem.data;
                else
                    if (dataArmourStand != null)
                        return dataArmourStand.fixer ? (def.type() == dataArmourStand.data.type() ? dataArmourStand.data : def) : dataArmourStand.data;
            }
     ...
     * Data Matchers
     * 
     * The following methods are the matchers to match and load display data of that
     * item, after that, a DisplayData object will be created with its data wrapped
     * in.
     */
    ...
    private static class DisplayDataMatchesResult {
        ...
        /**
         * Indicates whether this data is a display fixer. A fixer menas only when the
         * default display type matches, this data will be applied.
         * 
    ...
    @Nullable
    private static DisplayDataMatchesResult matchDataFor(@NotNull ConfigurationSection conf, @NotNull String rootType, @NotNull ItemStack item, @NotNull DisplayType displayType) {
    ...
    @Nullable
    private static DisplayDataMatchesResult matchDataFor(@NotNull List<?> specifics, @NotNull ItemStack item, @NotNull DisplayType displayType) {
