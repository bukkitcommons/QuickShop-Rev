...
public class QuickShopCommandManager implements TabCompleter, CommandExecutor {
    ...
    private void sendHelp(@NotNull CommandSender sender, @NotNull String commandLabel) {
        ...
        for (CommandData container : commands) {
            ...
            sender.sendMessage(ChatColor.GREEN + "/" + commandLabel + " " + container.label() + ChatColor.YELLOW + " - " + Shop.getLocaleManager().get("command.description." + container.label(), player ? (Player) sender : null));
        }
    }
    ...
    public List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (args.length == 1) {
            ...
            for (CommandData data : commands) {
                ...
                if (data.label().startsWith(subLabel))
                    tabCommands.add(data.label());
            }
            return tabCommands;
        }
