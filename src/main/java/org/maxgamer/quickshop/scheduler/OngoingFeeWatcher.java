            ...
            if (!shop.unlimited() || !ignoreUnlimited) {
                ...
                if (!success) {
                    ...
                    try {
                        ...
                        QuickShop.instance().getEconomy().deposit(Bukkit.getOfflinePlayer(BaseConfig.taxAccount).getUniqueId(), cost);
        ...
        Bukkit.getScheduler().runTask(Shop.instance(), () -> {
            try {
                ...
                if (!shop.unlimited() || !BaseConfig.ignoreUnlimitedMessages)
                    Shop.getMessager().send(shop.moderators().getOwner(), Shop.getLocaleManager().get("shop-removed-cause-ongoing-fee", Bukkit.getOfflinePlayer(shop.moderators().getOwner()), "World:" + shop.world() + " X:" + shop.x() + " Y:" + shop.y() + " Z:" + shop.z()));
