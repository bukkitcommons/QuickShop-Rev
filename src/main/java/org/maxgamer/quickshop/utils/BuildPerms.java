...
public class BuildPerms {
    ...
    public boolean canBuild(@NotNull Player player, @NotNull Block block) {
        ...
        facadePlace = new BlockPlaceEvent(block, air, block, new ItemStack(signType), player, true, EquipmentSlot.HAND);
        // Call for event for protection check start
        
        Bukkit.getPluginManager().callEvent(new ShopProtectionCheckEvent(block.getLocation(), player, ProtectionCheckStatus.BEGIN, facadePlace));
        Bukkit.getPluginManager().callEvent(facadePlace);
        // Call for event for protection check end
        Bukkit.getPluginManager().callEvent(new ShopProtectionCheckEvent(block.getLocation(), player, ProtectionCheckStatus.END, facadePlace));
